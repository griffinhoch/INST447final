...
One important part of this project was checking whether there were any patterns in the dataset that could affect how the results should be interpreted. We checked for missing values in the final dataset. There were no missing values because teams without certain situations were filled with zeros during the merge process. We also examined whether all NFL teams were represented in the dataset and while every team appears in the final dataset not all have the same number of offensive plays. Some teams ran more plays because of differences in offensive pace, game script, time of possession, and overtime games. This means that some teams had more opportunities for situations like fourth down attempts or red zone plays. For example, Cleveland had over 1,500 offensive plays during the season while Las Vegas had around 1,3000. This imbalance shows that some teams had more chances to accumulate statistics. 

Moving on, we investigated whether opportunity volume could affect efficiency metrics. For example, teams with more red zone opportunities may not have higher touchdown efficiency. Similarly, teams with more fourth down attempts are not automatically more successful on fourth down. Our correlation analysis between red zone plays and red zone touchdown rate produced a correlation coefficient of 0.273 which indicates a weak positive relationship. Teams with more opportunities in the red zone sometimes performed better but the relationship wasn't strong enough to suggest that opportunity volume alone determines efficiency. There are also possible sources of bias in how NFL play by play data is generated. The original data comes from NFL game tracking and live game charting. Human scorekeepers and statisticians determine details like play descriptions, penalties, and classification of certain plays. While nflverse standardizes the data, there may still be inconsistencies in how some plays are recorded. 
...
plt.figure()
top_10_pass_rate["early_down_pass_rate_pct"].plot(kind="bar")
plt.xticks(ticks=range(10), labels=top_10_pass_rate["posteam"])
...
Our second main question dealt with the teams that attempted fourth downs most frequently. As seen earlier in our analysis, the Carolina Panthers and Detroit Lions were the top two teams, followed by the New York Giants. The Panthers attempted the most fourth downs by a solid margin. 

```{python}
top10_fourth_down = final_dataset[final_dataset["fourth_down_attempt_rank"] <= 10].sort_values("fourth_down_attempt_rank")
plt.figure()
top10_fourth_down["fourth_down_attempts"].plot(kind="bar")
plt.xticks(ticks=range(10), labels=top10_fourth_down["posteam"])
...
plt.figure()
top10_scoring_drives["avg_plays_per_scoring_drive"].plot(kind="bar")
plt.xticks(ticks=range(10), labels=top10_scoring_drives["posteam"])
...
plt.figure()
top10_redzone["red_zone_td_rate_pct"].plot(kind="bar")
plt.xticks(ticks=range(10), labels=top10_redzone["posteam"])
